    ...
    def makeup(input, dup_rm_enabled=True, format_name_enabled=True, speedtest=True): # 对节点进行区域的去重筛选和重命名以及测速，输出 YAML 文本 
        ...
        if speedtest: # 测速
            ...
            for proxy in proxies_list:
                ...
                if ping_result[0] >= 0.05 or ping_result_g[0] >= 0.05 or ping_result_y[0] >= 0.05 or ping_result_w[0] >= 0.05:
                    proxies_list.remove(proxy)
                elif ping_result[1] < 1 or ping_result_g[1] < 1 or ping_result_y[1] < 1 or ping_result_w[1] < 1:
                    proxies_list.remove(proxy)
